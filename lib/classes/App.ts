...
export default class App extends MiddlewareManager {
  ...
  public start(httpPort: number, started?: () => void): void {
    ...
    try {
      ...
      throw new Error(
        `\x1b[32m[Zebra]\x1b[0m: An unexpected error occued while starting HTTP server. \n${err}`
      );
    }
    ...
    if (this.keys) {
      try {
        ...
        throw new Error(
          `\x1b[32m[Zebra]\x1b[0m: An unexpected error occured while starting HTTPS server. \n${err}`
        );
      }
    } else {
      canRun += 1;
