...
export default class RouteManager {
  ...
  async _handleRoute(req: Request, res: Response, staticRoutes: StaticRoute[]) {
    ...
    if (route?.getAvailableMethods().includes(req.method)) {
      try {
        if (this.debug)
          console.log(
            `\x1b[32m[Zebra]\x1b[0m: Recieved, a request executing route ${route.path}.`
          );
  ...
  async _handleRouteError(
    ...
    console.log(
      `\x1b[32m[Zebra]\x1b[0m: \x1b[31mEncountered an error on ${req.method} ${req.path}, open route on browser to see information about the error.\x1b[0m`
    );
